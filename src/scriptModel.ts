...
import { IBuiltInParameterName, ParameterType, IErrorMessage, ValidationOptions, NotifyScriptModelChanged, IParameterState, IScriptModelState } from "./commonModel"
...
export class ScriptModel {
    ...
    private propertyChangedEvent = new SimpleEventDispatcher<Partial<IScriptModelState>>();

    public generateBashAndUpdateAll = () => {


        if (this.propertyChangedEvent !== undefined && this.generateBashScript) {
            this.JSON = this.stringify();
            this.bashScript = this.toBash();
        }
    ...
    private getValidationErrors = (options: ValidationOptions): IErrorMessage[] => {
        ...
        for (let param of this.parameters) {
            ...
            if (param.type === ParameterType.Logging && this.ScriptName === "") {
    ...
    public get onScriptModelChanged(): ISimpleEvent<Partial<IScriptModelState>> {
    ...
    public toBash = (): string => {
        ...
        try {
            ...
            for (let param of this.parameters) {
                ...
                if (this._builtInParameters.InputFile !== undefined && param.variableName !== "inputFile") {
                    // parse input file
            ...
            if (this._builtInParameters.Logging !== undefined) {
            ...
            let inputOverridesRequired: string = (this._builtInParameters.InputFile !== undefined) ? "echoWarning \"Parameters can be passed in the command line or in the input file. The command line overrides the setting in the input file.\"" : "";
            sbBashScript = sbBashScript.replace("__USAGE_INPUT_STATEMENT__", inputOverridesRequired);

            if (this._builtInParameters.InputFile !== undefined) {
        ...
        try {
            switch (type) {
                ...
                case ParameterType.Delete:
                    ...
                    break;
                case ParameterType.InputFile:
                    if (this.BuiltInParameters.InputFile !== undefined) {
                        this.deleteParameter(this.BuiltInParameters.InputFile);
                    }
                    ...
                    parameterModel.collapsed = true;
                    parameterModel.type = ParameterType.InputFile;
                    this.BuiltInParameters.InputFile = parameterModel;
                    break;
                case ParameterType.Logging:
                    if (this.BuiltInParameters.Logging !== undefined) {
                        this.deleteParameter(this.BuiltInParameters.Logging);
                    }
                    ...
                    parameterModel.default = "\"./\"";
                    parameterModel.type = ParameterType.Logging;
                    parameterModel.requiresInputString = true;
                    parameterModel.requiredParameter = false;
                    parameterModel.valueIfSet = "$2";
                    parameterModel.collapsed = true;
                    parameterModel.type = ParameterType.Logging;
                    this.BuiltInParameters.Logging = parameterModel;
                    break;
                case ParameterType.Verbose:
                    if (this.BuiltInParameters.Verbose !== undefined) {
                        this.deleteParameter(this.BuiltInParameters.Verbose);
                    }
                    parameterModel.default = "false";
                    ...
                    parameterModel.collapsed = true;
                    parameterModel.type = ParameterType.Verbose;
                    this.BuiltInParameters.Verbose = parameterModel;
                    break;
                default:
                    break;
        ...
        try {
            ...
            for (let p of objs.Parameters) {
                ...
                if (model.type !== "Custom") {
                    this.BuiltInParameters[model.type] = model;
                }
                /* switch (model.type) {
                    case ParameterType.Create:
                        this.BuiltInParameters.Create = model;
                        break;
    ...
    private onParameterChanged = (model: ParameterModel, name: keyof IParameterState) => {
